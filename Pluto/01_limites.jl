...
# ╔═╡ cbc23758-58e8-11ef-2d02-d9880cf966aa
TableOfContents()

# ╔═╡ e7a9b17f-da14-4001-8df7-454a66c6b4ac
md"# Límites"

# ╔═╡ d022d477-d390-4762-b8e3-2c681165623a
md"## Definición intuitiva del límite"
...
md"""
**Ejemplo 1**. Determina el límite cuando $x$ tiende a 3 de la función 
...
md"""
+ Se observa que para valores de $x$ muy cercanos a 3 por la izquierda, $f(x)$ tiende a 6.
+ Lo mismo ocurre para valores cercanos por la derecha.

Es decir, $\lim_{x \rightarrow 3^{-}} f(x) = 6$ y $\lim_{x \rightarrow 3^{+}} f(x) = 6$, por lo tanto $\lim_{x \rightarrow 3} f(x)=6.$
...
begin
	...
	x1 = range(-10, stop=-2, length=500)
	x2 = range(-2.001, stop=10, length=500)

	# Graficar las dos partes de la función
	plot(x1, g1.(x1), linewidth=2, color=:blue, xlabel="x", ylabel="f(x)", label = false, framestyle=:origin)
	plot!(x2, g2.(x2), linewidth=2, color=:red, label = false)

	# Añadir un punto sólido en x=-2 para g1 y un punto vacío para g2
	scatter!([-2], [g1(-2)], label="", color=:blue, marker=:circle, ms=8)
	scatter!([-2], [g2(-2)], label="", color=:white, marker=:circle, ms=8, msc=:red, markerstrokewidth=2)
end

# ╔═╡ 63c19177-168a-49aa-9241-4613a40df28e
md"""
Aquí tenemos que $\lim_{x \rightarrow -2^{-}} f(x) = 8$ y $\lim_{x \rightarrow -2^{+}} f(x)=4$.

Entonces $\lim_{x \rightarrow -2^{-}} f(x) \neq \lim_{x \rightarrow -2^{+}} f(x)$.

Por tanto, $\lim_{x \rightarrow -2} f(x)$ no existe.
"""
...
$\lim_{\theta \rightarrow 0} \frac{sen \theta}{\theta}$
"""

# ╔═╡ f22ff197-a3e1-496d-ad15-9d235f82b46e
begin
	# Definir la función f(θ)
	function f3(θ)
  	  return sin(θ) / θ
	end

	# Definir el rango de θ alrededor de 0, excluyendo exactamente θ=0 para evitar la división por cero
	θ = range(-15.0, stop=15.0, length=1000)

	# Graficar la función f(θ)
...
md"## Definición formal de límite"

# ╔═╡ d28fa0a0-63b0-4a92-abf4-46cb8800bf41
md"""
El $\lim_{x \rightarrow a} f(x) = L$, si para todo $\epsilon > 0$ existe un $\delta > 0$, tal que:
